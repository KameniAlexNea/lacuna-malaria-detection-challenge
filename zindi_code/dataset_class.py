...
from zindi_code import CLS_MAPPER, DATA_FOLDER, RESIZE_TO_H, RESIZE_TO_W
from zindi_code.utils import (collate_fn, get_inference_transform,
                              get_test_transform, get_train_transform,
                              get_valid_transform, mkdir)
...
class CustomDataset(Dataset):
    def __init__(
        ...
        data = load_and_format(
            os.path.join("/workspace", DATA_FOLDER, file_name + ".csv")
        )

        print(
            "Number of rows without boxes: ",
            (data["category_id"] == len(CLS_MAPPER)).sum(),
        )
        ...
        data = (
            data.groupby("image_id")
            .agg({"bbox": list, "category_id": list})
            .reset_index()
        )
    ...
    def __getitem__(self, idx):
        ...
        if boxes is None:
            ...
            sample = self.transforms(
                image=image_resized, bboxes=np.array([]), labels=np.array([])
            )
...
def get_dataloader(
    ...
    if inference_mode:
        dataset = create_inference_dataset(
            save_picture=save_picture, resize_img=(batch_size != 1)
        )
    else:
    ...
    if inference_mode:
        ...
        print(
            f"Number of {'training' if is_training else 'validation'} samples: {len(dataset)}"
        )
...
def get_mean_var_train_dataset():
    ...
    train_loader = get_dataloader(train=True, batch_size=5000, num_workers=4)
...
def get_mean_var_area_dataset():
    ...
    print(
        "Box to Img ratio (%)|| Mean: {}, Std: {}".format(
            box_img_ratio_mean, box_img_ratio_std
        )
    )
